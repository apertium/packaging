...
print `svn export https://svn.code.sf.net/p/apertium/svn/trunk/apertium/configure.ac`;
my $major = 0;
my $minor = 0;
my $patch = 0;
my $revision = `svn log -q -l 1 https://svn.code.sf.net/p/apertium/svn/trunk/apertium/ | egrep -o '^r[0-9]+' | egrep -o '[0-9]+'` + 0;
{
	local $/ = undef;
...
foreach my $distro (keys %distros) {
	...
	my $chlog = <<CHLOG;
apertium ($chver) $distro; urgency=low

  * Automatic build - see changelog via: svn log https://svn.code.sf.net/p/apertium/svn/trunk/apertium/

 -- $opts{e}  $date
CHLOG

	`cp -al 'apertium-$version' 'apertium-$chver'`;
	unlink "apertium-$chver/debian/changelog";
	open FILE, ">apertium-$chver/debian/changelog" or die "Could not write to debian/changelog: $!\n";
	print FILE $chlog;
	close FILE;
	print `dpkg-source '-DMaintainer=$opts{m}' '-DUploaders=$opts{e}' -b 'apertium-$chver'`;
	chdir "apertium-$chver";
	print `dpkg-genchanges -S -sa '-m$opts{m}' '-e$opts{e}' > '../apertium_$chver\_source.changes'`;
	chdir '..';
	print `debsign 'apertium_$chver\_source.changes'`;
}
