...
wget https://svn.code.sf.net/p/apertium/svn/branches/packaging/trunk/lttoolbox/make-deb-source.pl -O make-deb-source.pl
chmod +x *.pl
...
mkdir -p /home/apertium/public_html/apt/logs/lttoolbox/
...
do
	...
	do
		...
		time cowbuilder --build *$DISTRO*.dsc --basepath /var/cache/pbuilder/base-$DISTRO-$ARCH.cow/ 2>&1 | tee /home/apertium/public_html/apt/logs/lttoolbox/$DISTRO-$ARCH.log
	done
done
